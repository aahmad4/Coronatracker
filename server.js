...
require("http").createServer((req,res)=>{
  ...
  if(["","/"].includes(req.url)){
    ...
    var execProcess=exec("python3 server.py \""+req.url.substr(3).toLowerCase().replace(/%20/g,' ').replace(/\"/g,'')+"\"");
    ...
    if(fs.existsSync('public'+req.url))res.write(fs.readFileSync('public'+req.url,'utf8'));
  }
  res.end();
}).listen(8080);